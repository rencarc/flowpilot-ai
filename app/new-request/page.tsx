...
export default async function NewRequestPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <AppShell>
      <PageHeader title="New request" subtitle="Create a persisted Supabase case. AI analysis remains mocked until Step 5." action={<Link className="secondary-btn" href="/cases">Back to cases</Link>} />
